...
class CombinedDateTimePickerDemo extends StatefulWidget {
  const CombinedDateTimePickerDemo({super.key});
  @override
  _CombinedDateTimePickerDemoState createState() => _CombinedDateTimePickerDemoState();
}

class _CombinedDateTimePickerDemoState extends State<CombinedDateTimePickerDemo> {
  ...
  Future<void> _selectDateTime(BuildContext context) async {
    ...
    if (pickedDate != null) {
      final TimeOfDay? pickedTime = await showTimePicker(
        context: context,
        initialTime: TimeOfDay.now(), // Use the current time as the starting point
      );
      if (pickedTime != null) {
        setState(() {
